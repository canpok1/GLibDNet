...
namespace TestWindowForm
{
	class TestScene1 : Scene
	{
		...
		public TestScene1( SceneParameter param ) : base( param )
		{
			...
			this.sounds.Add(
				new SoundStreamProperty(
					Properties.Resources.se_maoudamashii_system49,
					"se_maoudamashii_system49",
					SoundType.WAVE));
		...
		public override UpdatedStateEnum syncronousUpdate()
		{
			...
			if (InputManager.getInstance().isOneTimeModeKeyPressed(InputManager.KeyCodeEnum.MR) == true)
			{
				SoundManager.getInstance().play(this.sounds[0]);
			}
